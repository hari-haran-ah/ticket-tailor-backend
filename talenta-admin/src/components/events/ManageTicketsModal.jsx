...
export default function ManageTicketsModal({ isOpen, onClose, client_id, event, groups, onCreated, editData = null, mode = 'create' }) {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <div className="card w-full max-w-lg bg-white dark:bg-[#212121] border-gray-300 dark:border-white/10 shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="p-4 border-b border-gray-300 dark:border-white/5 flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-white/40">{event.name}</p>
                    </div>
                    <button onClick={onClose} className="text-gray-400 dark:text-white/40 hover:text-gray-900 dark:hover:text-white transition-colors"><X size={20} /></button>
                </div>
                    ...
                    {activeTab === 'ticket' ? (
                        <form onSubmit={handleTicketAction} className="space-y-4">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold text-gray-500 dark:text-white/40 uppercase tracking-widest ml-1">Ticket Name</label>
                                <input
                                    required
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-bold text-gray-500 dark:text-white/40 uppercase tracking-widest ml-1">Price (£)</label>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold text-gray-500 dark:text-white/40 uppercase tracking-widest ml-1">Group (Optional)</label>
                                {groups && groups.length > 0 ? (
                                    ...
                                    >
                                        <option value="" className="bg-white dark:bg-[#212121] text-gray-400 dark:text-white/50 italic">Assign to a Group...</option>
                                        {groups.map(g => (
                                            <option key={g.id} value={g.id} className="bg-white dark:bg-[#212121] text-gray-900 dark:text-white">
                                                {g.name}
                                            </option>
                                        ))}
                                    </select>
                            ...
                            <button disabled={loading} className="btn-primary w-full py-3.5 flex items-center justify-center gap-2 mt-4 transition-all active:scale-95 shadow-lg shadow-black/20 dark:shadow-white/20">
                        ...
                        <form onSubmit={handleGroupAction} className="space-y-5">
                            <div className="space-y-1.5">
                                <label className="text-[10px] font-bold text-gray-500 dark:text-white/40 uppercase tracking-widest ml-1">Group</label>
                                <input
                                    required
                                    className="input-field"
                            ...
                            <button disabled={loading} className="btn-primary w-full py-3.5 flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg shadow-black/20 dark:shadow-white/20">
                                {loading ? <div className="animate-spin h-4 w-4 rounded-full border-b-2 border-white" /> : isEdit ? <Save size={18} /> : <Plus size={18} />}
